...
if st.session_state.get("sh_ready"):
    ...
    if selected_shop:
        ...
        if not shop_abc_sel.empty:
            ...
            # ── Download Excel for this shop ─────────────────────────────────
            st.markdown("---")
            st.markdown("**Завантажити аналіз магазину:**")
            _dl1, _dl2 = st.columns(2)
            with _dl1:
                if st.button("📊 Excel — ABC магазину", key="btn_shop_xl", use_container_width=True):
                    with st.spinner("Формування Excel..."):
                        try:
                            import io
                            from openpyxl import Workbook
                            ...
                            wb2=Workbook()
                            ws_abc=wb2.active; ws_abc.title=selected_shop[:28]; ws_abc.freeze_panes='A3'
                            _H=['№','Артикул','Група','Назва','ВП (USD)','Частка %','Кумул. %','ABC']
                            _W=[6,16,28,52,18,11,11,7]
                            for ci,(h,w) in enumerate(zip(_H,_W),1):
                                c=ws_abc.cell(1,ci,h); _s(c,bold=True,fg='FFFFFF',bg=_CH,align='center',size=9)
                                ws_abc.column_dimensions[get_column_letter(ci)].width=w
                            ws_abc.row_dimensions[1].height=20
                            _row=2
                            for abc in ['A','B','C']:
                                ...
                                ws_abc.row_dimensions[_row].height=14
                                _c=ws_abc.cell(_row,1,f'  {_lbl}'); _c.font=Font(name='Arial',bold=True,color=_fg,size=9)
                                _c.fill=_f(_bg); _c.alignment=Alignment(horizontal='left',vertical='center')
                                ws_abc.merge_cells(start_row=_row,start_column=1,end_row=_row,end_column=len(_H)); _row+=1
                                for _,_rec in _gd.iterrows():
                                    ...
                                    ws_abc.row_dimensions[_row].height=13
                                    for ci,val in enumerate(_vals,1):
                                        _c=ws_abc.cell(_row,ci,val)
                            ...
                            import traceback; st.error(f"Помилка: {e}"); st.code(traceback.format_exc())
            with _dl2:
                if (st.session_state.get("sh_shop_xl_data") and
                        st.session_state.get("sh_shop_xl_name") == selected_shop):
                    st.download_button(
                        "⬇️ Завантажити Excel",
                    ...
                    st.info("Натисніть кнопку зліва")
